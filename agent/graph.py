...
def analyze_question_and_decide_action(state: AgentState, llm_with_retriever_tools: ChatDeepSeek) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not conversation_messages:
            ...
            return {"messages": [AIMessage(content="I need a question to be able to help you.")]}
        ...
        return {"messages": [AIMessage(content="I encountered an error while processing your question.")]}
...
def evaluate_document_relevance(state: AgentState) -> Dict[str, str]:
    ...
    try:
        ...
        document_grader_llm = ChatDeepSeek(
...
def generate_final_answer(state: AgentState) -> Dict[str, List[BaseMessage]]:
    ...
    try:
        ...
        if not user_question:
            ...
            return {"messages": [AIMessage(content="I need a clear question to be able to answer.")]}
        ...
        answer_generator_llm = ChatDeepSeek(
            model=CONFIG.DEEPSEEK_LLM_MODEL, 
            temperature=0, 
            api_key=CONFIG.DEEPSEEK_API_KEY,
        )
        ...
        return {"messages": [AIMessage(content="I encountered an error while generating the answer. Please try again.")]}
